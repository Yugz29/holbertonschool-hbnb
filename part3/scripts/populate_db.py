...
def init_database():
    """Initialize database file and directory with proper permissions"""
    db_path = Path(__file__).resolve().parent.parent / "sql" / "hbnb.db"
    ...
    os.chmod(db_path.parent, 0o755)  # drwxr-xr-x for directory
    os.chmod(db_path, 0o644)         # -rw-r--r-- for file
...
def clear_database():
    ...
    try:
        ...
        # Clear other tables
        for model in [Review, Place, Amenity, User]:
            try:
...
def main():
    ...
    # Update the database URI to use our initialized path
